...
def resolver_sistema(presupuesto, alpha, f_dem, f_esp, f_gen, f_int):
    ...
    for j in niveles:
        for i in perfiles:
            if i != 'Interno_Rotativo':
                vars_x[(i, j)] = m.addVar(vtype=GRB.INTEGER, lb=0)
        vars_y[j] = m.addVar(vtype=GRB.INTEGER, lb=0)

    costo_tot = gp.quicksum(c_act[i] * vars_x[i, j] for j in niveles for i in perfiles if i != 'Interno_Rotativo')
    costo_tot += gp.quicksum(c_act['Interno_Rotativo'] * vars_y[j] for j in niveles)
    m.setObjective(costo_tot, GRB.MINIMIZE)

    for j in niveles:
        for i in perfiles:
            d_ajust = demanda_base[j][i] * f_dem
            if i != 'Interno_Rotativo':
                m.addConstr(capacidad_base[i] * (planta[j][i] + vars_x[i, j]) >= d_ajust)
            else:
                m.addConstr(capacidad_base[i] * vars_y[j] >= d_ajust)
        m.addConstr(vars_y[j] <= alpha * gp.quicksum(planta[j][i] + vars_x[i, j] for i in perfiles if i != 'Interno_Rotativo'))

    m.addConstr(m.getObjective() <= presupuesto)
    m.optimize()
    ...
    if m.status == GRB.OPTIMAL:
        ...
        for j in niveles:
            for i in perfiles:
                val = int(vars_x[i, j].X) if i != 'Interno_Rotativo' else int(vars_y[j].X)
                p_v = planta[j][i] if i != 'Interno_Rotativo' else 0
                costo_perfil = c_act[i]
                res.append({
                    'Nivel': j, 'Perfil': i, 'Planta': p_v, 'Ocasional': val, 'Total': p_v + val,
                    'Capacidad': (p_v + val) * capacidad_base[i], 
                    'Demanda': demanda_base[j][i] * f_dem,
                    'Cobertura_%': ((p_v + val) * capacidad_base[i] / (demanda_base[j][i] * f_dem)) * 100,
                    'Costo_Ocasional': val * costo_perfil
                })
        
        df_inv = pd.DataFrame({
            'Categoría': ['Especialistas Oc.', 'Generales Oc.', 'Internos Rot.'],
            'Inversión': [
                sum(c_act['Medico_Especialista'] * vars_x['Medico_Especialista', j].X for j in niveles),
                sum(c_act['Medico_General'] * vars_x['Medico_General', j].X for j in niveles),
                sum(c_act['Interno_Rotativo'] * vars_y[j].X for j in niveles)
            ]
        })
        return True, m.objVal, pd.DataFrame(res), df_inv
