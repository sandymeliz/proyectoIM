## Optimización de Personal en Hospitales Públicos del Ecuador

Este proyecto implementa un modelo de Programación Lineal Entera Mixta (MILP)
para la asignación óptima de personal sanitario, resuelto mediante el solver
comercial Gurobi.

### Versión DEMO (Pública)
La versión disponible en línea corresponde a una versión demostrativa
haciendo uso de Pulp(un solver gratuito), desarrollada únicamente con fines de visualización
y análisis interactivo.

### Versión REAL (Gurobi)
La versión completa del modelo (`app.py`) requiere:
- Licencia académica Gurobi
- Ejecución local o en servidor autorizado

